#!/usr/bin/env python3
"""
Timur Translator Realtime - macOS build.

macOS microphone OR BlackHole virtual system-audio input -> OpenAI Realtime Translation -> live subtitles.

Key reliability changes:
- validates the selected CoreAudio input before leaving the setup screen;
- opens microphones at a device-supported native sample rate;
...
- captures YouTube, Zoom, Meet and interview audio through BlackHole 2ch or another virtual input;
...
- preserves themes, transparency, overlay mode and Turkish target-language support.
"""
...
PLATFORM_LABEL = "macOS" if IS_MACOS else "Windows" if IS_WINDOWS else sys.platform

AUDIO_SOURCE_OPTIONS = {
    (
        "System audio · YouTube / Zoom / Meet / interview · BlackHole"
        if IS_MACOS
        else "System audio · YouTube / Zoom / Meet / interview (recommended)"
    ): "system",
    "Microphone · translate my own voice": "microphone",
}
SYSTEM_AUDIO_SAMPLE_RATE = 48_000
SYSTEM_AUDIO_LABEL = (
    "System audio · BlackHole virtual input"
    if IS_MACOS
    else "System audio loopback · default Windows playback device"
)
...
def find_supported_input_config(device: DeviceInfo) -> AudioInputConfig:
    """Return an input format PortAudio can actually open on this audio device."""
    import pyaudio
    ...
    raise RuntimeError(
        f"{PLATFORM_LABEL} не смогла открыть выбранный аудиовход. Выбери другое устройство "
        "и проверь разрешение на доступ к микрофону в системных настройках."
    )
...
def find_best_supported_input_config(
    ...
    """Probe inputs and return the first microphone PortAudio can open.

    The preferred device is tested first. When it cannot open, the function
    transparently falls back to another usable input.
    """
    failures: list[str] = []
    ...
    raise RuntimeError(
        f"{PLATFORM_LABEL} не смогла открыть ни один подходящий микрофон. Проверь доступ к микрофону "
        "в системных настройках и закрой приложения, которые могли занять аудиовход.\n\n"
        f"Проверенные входы:\n{detail}"
    )
...
def _looks_like_macos_system_audio(device: DeviceInfo) -> bool:
    lowered = _device_text(device)
...
def find_macos_system_audio_input_config() -> AudioInputConfig:
    ...
    if not candidates:
        raise RuntimeError(
            "Системный звук macOS нельзя захватить напрямую без виртуального аудиодрайвера. "
            "Установи BlackHole 2ch, создай Multi-Output Device в Audio MIDI Setup и нажми REFRESH."
        )
    failures: list[str] = []
    for device in candidates:
        try:
            config = find_supported_input_config(device)
            logging.info("Selected macOS virtual system-audio input: %s", config.description)
            return config
        except Exception as exc:
            failures.append(f"[{device.index}] {device.name}: {exc}")
    raise RuntimeError(
        "BlackHole найден, но macOS не смогла открыть его как аудиовход. Проверь Audio MIDI Setup, "
        "выбери Multi-Output Device для вывода звука и перезапусти приложение.\n\n"
        + "\n".join(failures[-5:])
    )
...
def find_system_audio_input_config() -> AudioInputConfig:
    """Probe the platform-specific system-audio route before replacing the setup window."""
    if IS_MACOS:
        return find_macos_system_audio_input_config()
...
class RealtimeTranslationClient:
    ...
    def start(self) -> None:
        ...
        if self.audio_source_mode == "system" and IS_MACOS:
            capture_target = self._mac_system_audio_capture_loop
            capture_name = "mac-system-audio"
        elif self.audio_source_mode == "system":
            capture_target = self._system_audio_capture_loop
            capture_name = "system-loopback"
        else:
            capture_target = self._capture_loop
            capture_name = "mic-capture"
        self.threads = [
        ...
        try:
            while self.running.is_set():
                try:
                    ...
                    self.audio_input = cfg
                    self.callbacks.on_input_changed(cfg)
                    self.callbacks.on_status("Listening · macOS system audio through BlackHole", GREEN)
                    logging.info("macOS virtual system-audio input opened: %s", cfg.description)
                    while self.running.is_set():
                        raw = stream.read(cfg.frames_per_buffer, exception_on_overflow=False)
                        payload = silence if self.paused.is_set() else normalize_input_pcm16(raw, cfg)
                        self._enqueue_audio(payload)
                    ...
                    if not self.running.is_set():
                        return
                    logging.exception("macOS system-audio capture interrupted")
                    self.callbacks.on_error(f"System audio reconnecting: {exc}")
                    self.callbacks.on_status("Searching for BlackHole system audio…", AMBER)
                    time.sleep(0.9)
                    try:
                        cfg = find_macos_system_audio_input_config()
                    except Exception as fallback_exc:
                        logging.exception("macOS system-audio recovery failed")
                        self.callbacks.on_error(f"BlackHole is not ready: {fallback_exc}")
                        time.sleep(1.2)
                finally:
                    self._close_input_stream(stream)
                    stream = None
            ...
            self._close_input_stream(stream)
            pa.terminate()
            logging.info("macOS system-audio capture stopped")
    ...
    def _headset_monitor_loop(self) -> None:
        """Watch for headset hot-plug and default-input changes off-thread."""
        while self.running.is_set():
                ...
                for device in sorted(headset_candidates, key=lambda item: _device_score(item, True), reverse=True):
                    ...
                    if (
                        ...
                        try:
                            ...
                            logging.warning("Changed default input could not open: %s", default_device.display_name)
    ...
    def _connect(self):
        ...
        self.callbacks.on_status(
            "Listening · system audio ready" if self.audio_source_mode == "system" else "Listening · microphone ready",
            GREEN,
        )
        return ws
...
class SetupWindow:
    ...
    def _build(self) -> None:
        ...
        tk.Label(
            self.root,
            text=("OpenAI Realtime · macOS microphone + BlackHole system audio" if IS_MACOS else "OpenAI Realtime · Windows microphone + system-audio loopback"),
            font=(FONT, 12), bg=BG, fg=TEXT2,
        ).pack(pady=(0, 18))
        ...
        tk.Label(
            form,
            text=(
                "For YouTube or an online interview choose System audio. On macOS this reads BlackHole 2ch, so create a Multi-Output Device with BlackHole and your headphones or speakers first. Choose Microphone only for your own voice."
                if IS_MACOS
                else "For YouTube or an online interview choose System audio. It captures what you hear in speakers or headphones. Choose Microphone only when you want to translate your own voice."
            ),
        ...
        tk.Label(
            form,
            text=(
                "Leave AUTO selected. The app prefers a connected headset microphone and falls back to the Mac microphone when needed."
                if IS_MACOS
                else "Leave AUTO selected. The app tests Windows driver duplicates, picks a headset microphone first when one is connected, and falls back to the laptop microphone when needed."
            ),
            font=(FONT, 9), bg=BG, fg=TEXT3, justify="left", wraplength=520,
    ...
    def _refresh_devices(self) -> None:
        ...
        names = [f"[AUTO] Headset microphone first, then {PLATFORM_LABEL} default"] + [device.display_name for device in self.devices]
    ...
    def _start(self) -> None:
        ...
        try:
            if source_mode == "system":
                self.status.config(
                    text=("Checking BlackHole system-audio input…" if IS_MACOS else "Checking Windows system audio loopback…"),
                    fg=AMBER,
                )
                ...
                self.status.config(text=f"Checking {PLATFORM_LABEL} microphone inputs…", fg=AMBER)
                self.root.update_idletasks()
...
class TranslatorWindow:
    ...
    def _build(self, audio_input: AudioInputConfig, audio_source_mode: str) -> None:
        ...
        self.left_head = tk.Label(self.heads, text="ORIGINAL SOURCE SPEECH", font=(FONT, 10, "bold"), bg=p.surface2, fg=p.text3)
    ...
    def _apply_appearance(self) -> None:
        ...
        try:
            ...
            logging.warning("Could not apply one or more overlay attributes")
...
def main() -> None:
    ...
    if missing:
        ...
        messagebox.showerror(
            ...
            ("Run install_macos.command first.\n\nMissing: " if IS_MACOS else "Run install_windows.bat first.\n\nMissing: ") + ", ".join(missing),
        )
        return
